...
| Простой более 30 минут (`idleCapMs`) | Сторож автоматически засыпает (`idle cap reached`), предотвращая бесконечные ночные пинги. |
...
| `GEMINI_CACHE_GUARD_IDLE_CAP_MS` | `1800000` (30 мин) | Предел времени простоя, после которого сторож отключается. |
